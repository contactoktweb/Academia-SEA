...
import { deleteStudent } from "@/app/dashboard/alumnos/actions";
...
export function StudentDialog({ mode, student, trigger }: StudentDialogsProps) {
  ...
  if (mode === "delete") {
    const handleDelete = () => {
      startTransition(async () => {
        const promise = deleteStudent(student.id);
        await toast.promise(promise, {
          ...
          error: (err) => err.message || "Error al eliminar el alumno",
        });
      });
    };
      ...
      <AlertDialog open={open} onOpenChange={setOpen}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción eliminará permanentemente al alumno <strong>{student?.name}</strong> y todos sus datos asociados. No se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
