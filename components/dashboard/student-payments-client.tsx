...
export function StudentPaymentsClient({ initialData }: StudentPaymentsClientProps) {
  ...
  useEffect(() => {
    ...
    if (paymentStatus === "success" && sessionId) {
      startVerification(async () => {
        ...
        if (res.success) {
          toast.success("¡Pago procesado con éxito! Tu mensualidad ha sido acreditada.");
  ...
  const { enrollment, summary, installments, allPayments } = data;
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ─── 3. Grid de Mensualidades (Los 6 meses del curso) ─── */}
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-slate-200 pb-3">
          <div>
            <h2 className="text-xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
              <Calendar className="h-5 w-5 text-[#0066cc]" />
              Plan de Pagos y Mensualidades del Curso
            </h2>
            <p className="text-xs text-slate-500">
              Consulta las cuotas liquidadas (en verde) y paga las mensualidades pendientes con tarjeta.
            </p>
          </div>
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="flex items-center gap-1 font-semibold text-[#0066cc] bg-blue-50 px-2.5 py-1 rounded-full border border-blue-200">
              <span className="h-2 w-2 rounded-full bg-[#0066cc]" />
              Mes Actual
            </span>
            <span className="flex items-center gap-1 font-semibold text-slate-600 bg-slate-100 px-2.5 py-1 rounded-full border border-slate-200">
          ...
          {installments.map((inst) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  {/* Top Bar: Installment Number & Status Badge */}
                  ...
                  {/* Payment Reference details if paid */}
                  {isPaid && (
                ...
                {/* Bottom Action: Stripe Payment Button or Paid Confirmation */}
                <div className="mt-5">
                  {isPaid ? (
      ...
      {/* ─── 4. Historial General de Transacciones ─── */}
      <Card className="border-slate-200/80 shadow-xs">
        <CardHeader>
          <CardTitle className="text-lg font-bold flex items-center gap-2">
            <Receipt className="h-5 w-5 text-slate-500" />
            Historial de Recibos y Transacciones
          </CardTitle>
          <CardDescription>
            Registro de todos los pagos acreditados en el sistema de Academia SEA.
          </CardDescription>
        </CardHeader>
        <CardContent>
